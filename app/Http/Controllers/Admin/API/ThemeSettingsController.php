...
class ThemeSettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($options) && !empty($options)) {
            $result = ['settings' => $this->getSiteOptions(), 'page_title' => 'تنیظمات قالب'];
        }
...
//        return view('admin.list.settings',compact('settings'))->with('page_title','تنظمات قالب');
    ...
    public function store(Request $request)
    {
        if ($request->has('options')) {
            ...
            $result = ['status'=>true, 'message'=>'تنظیمات با موفقیت ذخیره شد.'];
        }
        else {
            $result = ['status'=>false, 'message'=>'داده ای ارسال نشده است.'];
        }
       return Response::json($result,200);
    }
    ...
    {
        $meta_value = static::$site_options;
        foreach ($meta_value as $index => $item) {

                if (count($meta_value[$index]) == 0) {
