...
//    public static $items_list='لیست نوشته ها';
    ...
    {
        return Response::json(['page_title' => 'افزودن مشتری']);
    }
    ...
    public function check_customer(Request $request)
    {
        ...
        if (isset($user)) {
            $result = ['status' => true, 'message' => 'اطلاعات وارد شده معتبر است.'];
        } else {
            $result = ['status' => false, 'message' => 'اطلاعات مشتری ثبت نشده است.'];
        }
    ...
    public function crm_request(Request $request, $user_id)
    {
        ...
        return Response::json(['data' => compact($compact), 'page_title' => 'درخواست ملک']);
...
//                        }
                ...
                return Response::json(['status' => true, 'crm_id' => $crm->idm, 'message' => 'درخواست شما با موفقیت ثبت شد']);
            } else {
                return Response::json(['status' => false, 'message' => 'مشتری جدید ایجاد نشد.']);
            }
        } else {
            return Response::json(['status' => false, 'message' => 'اطلاعاتی در درخواست ارسالی وجود ندارد.']);
        }
    }

    public function crm_request_update(Request $request, $crm_id)
    {
        $crm = CRM::findOrFail($crm_id);
...
//                        }
                ...
                return Response::json(['status' => true, 'crm_id' => $crm->id, 'message' => 'درخواست شما با موفقیت ویرایش شد']);
            } else {
                return Response::json(['status' => false, 'message' => 'عملیات به روزرسانی اطلاعات مشتری موردنظر ناموفق بود.']);
            }
        } else {
            return Response::json(['status' => false, 'message' => 'اطلاعاتی در درخواست ارسالی وجود ندارد.']);
        }
    ...
    public function crm_request_edit(Request $request, $crm_id)
    {
        ...
        return Response::json(['data' => compact($compact), 'page_title' => 'ویرایش درخواست ملک']);

    }

    public function add_new_customer(Request $request, $phone)
    {
        return Response::json(['phone_number' => compact(['phone']), 'page_title' => 'افزودن مشتری جدید']);
    }
    ...
    public function save_new_customer(Request $request, $phone)
    {
        ...
        if (isset($user)) {
            return Response::json(['status' => false, 'message' => 'مشتری موردنظر قبلا ثبت شده است.']);
        } else {
            ...
            if ($new_user) {
                ...
                $result = ['status' => true, 'new_user_id' => $new_user->id, 'message' => 'مشتری جدید با موفقیت ثبت شد.'];
            } else {
                $result = ['status' => false, 'message' => 'عملیات ثبت مشتری جدید ناموفق بود.'];
            }
        }
        return Response::json($result, 200);
    }
    ...
    public function show_all_customers(Request $request)
    {
        ...
        return Response::json(['data' => compact($compact), 'page_title' => 'همه مشتریان']);
    }
    ...
    public function show_my_customers(Request $request)
    {
        ...
        return Response::json(['data' => compact($compact), 'page_title' => 'مشتریان من']);
    }
    ...
    public function show_all_region_customers(Request $request)
    {
        ...
        return Response::json(['data' => compact($compact), 'page_title' => 'همه مشتریان منطقه']);
    }
    ...
    public static function showExactAdds($crm_id, $search_level = 0)
    {
        ...
        if (isset($crm_metas['land_type'])) {
            ...
            foreach ($land_type as $value) {
                $loop_land_type = Term::findOrFail($value);
...
//            dd(vsprintf(str_replace('?', '%s', $addSlashes), $posts_query->getBindings()));
    ...
    public function showSuggestedAdds(Request $request, $crm_id)
    {
        ...
        if (isset($adds_query)) {
            $adds = $adds_query->get();
            $compact = ['adds'];
            return Response::json(['data' => compact([$compact]), 'page_title' => 'آگهی های پیشنهادی']);
        } else {
            return Response::json(['status' => false, 'message' => 'برای این مشتری آگهی پیشنهادی موجود نیست.']);
        }
    }
...
EDT;
    ...
    public function crm_request_view($crm_id)
    {
        $crm = CRM::findOrFail($crm_id);
        if (!Auth::user()->can('adds_management') && $crm->expert->id !== Auth::id())
            return abort(403);
        ...
        $transactions = TransactionController::getTree();
        $compact = ['customer', 'ostans', 'termAndMetaHtml', 'customer_type', 'pay_method', 'purchage_state', 'talking', 'crm', 'land_types', 'transactions'];
        return Response::json(['data' => compact($compact), 'page_title' => 'مشاهده  درخواست ملک']);
