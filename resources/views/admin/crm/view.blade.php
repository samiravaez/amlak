            ...
            <div class="col-12">
                <table class="table table-bordered table-striped table-primary" id="user-regions">
                    ...
                    <tr>
                        <th>انتخاب محدوده</th>
                        <th>
                            <span class="fa fa-plus-circle text-success" id="add-user-regions"
                                  data-rows="{{isset($regions)?max(count($regions)-1,0):0}}"></span>
                        </th>
                    </tr>
                    </thead>
                    <tbody>
                                    ...
                                    <td>
                                        <div class="form-row" data-num="{{$cnt}}">
                                            <div class="form-group col-md-3" data-type="0">
                                                <label>استان</label>
                                                <select class="ostan-change-step select2"
                                                        name="region[{{$cnt}}][ostan]">
                                                    <option value="0">انتخاب همه</option>
                                                    @foreach($ostans as $option)
                                                        <option
                                                            value="{{$option->ID}}" {{(isset($region['ostan']) && $option->ID==$region['ostan']->ID)?'selected':''}}>{{$option->Title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                                ...
                                                <div class="form-group col-md-3" data-type="1">
                                                    ...
                                                    <select class="shahrestan-change-step select2"
                                                        ...
                                                        @foreach($region['ostan']->shahrestans as $option)
                                                            <option
                                                                value="{{$option->ID}}" {{(isset($region['shahrestan']) && $option->ID==$region['shahrestan']->ID)?'selected':''}}>{{$option->Title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endif
                                                ...
                                                <div class="form-group col-md-3" data-type="2">
                                                    ...
                                                    <select class="mantaghe-change-step select2"
                                                        ...
                                                        @foreach($region['shahrestan']->manategh as $option)
                                                            <option
                                                                value="{{$option->ID}}" {{(isset($region['mantaghe']) && $option->ID==$region['mantaghe']->ID)?'selected':''}}>{{$option->Title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endif
                                                ...
                                                <div class="form-group col-md-3" data-type="3">
                                                    ...
                                                    <select class="select2" name="region[{{$cnt}}][bakhsh]">
                                                        ...
                                                        @foreach($region['mantaghe']->bakhshs as $option)
                                                            <option
                                                                value="{{$option->ID}}" {{(isset($region['bakhsh']) && $option->ID==$region['bakhsh']->ID)?'selected':''}}>{{$option->Title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endif
                                ...
                                <div class="form-check form-check-inline mb-3">
                                    <label class="form-check-label">
                                        <input type="checkbox" class="form-check-input"
                                               value="{{$land_type['term_id']}}" name="metas[land_type][]"
                                               data-tab="land_type_{{$land_type['term_id']}}_tab" {{isset($crm_land_types) && in_array($land_type['term_id'],$crm_land_types)?'checked':''}}>
                                        {{$land_type['term_name']}}
                                    </label>
                                </div>
                            @endforeach
                            <a class="btn btn-outline-secondary float-right mx-3" data-toggle="collapse"
                               href="#land_types_detail" role="button" aria-expanded="false"
                               aria-controls="land_types_detail">
                                جستجوی پیشرفته
                            </a>
                        </div>
                    @endif
                        ...
                        <div class="form-group">
                            @foreach($transactions as $transaction)
                                <div class="form-check form-check-inline mb-3">
                                    <label class="form-check-label">
                                        <input type="checkbox" class="form-check-input"
                                               value="{{$transaction['term_id']}}" name="metas[transaction][]"
                                               data-tab="transaction_{{$transaction['term_id']}}_tab" {{isset($crm_transactions) && in_array($transaction['term_id'],$crm_transactions)?'checked':''}}>
                                        {{$transaction['term_name']}}
                                    </label>
                                </div>
                            @endforeach
                            <a class="btn btn-outline-secondary float-right mx-3" data-toggle="collapse"
                               href="#transactions_detail" role="button" aria-expanded="false"
                               aria-controls="transactions_detail">
                                جستجوی پیشرفته
                            </a>
                        </div>
                    @endif
                        ...
                        <div class="form-group">
                            ...
                            <select id="pay_method" class="form-control" name="main[pay_method]">
                                @foreach($pay_method as $index=>$value)
                                    <option
                                        value="{{$index}}" {{(isset($crm) && $crm->pay_method==$index)?'selected':''}}>{{$value}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="customer_type" class="form-control" name="main[customer_type]">
                                @foreach($customer_type as $index=>$value)
                                    <option
                                        value="{{$index}}" {{(isset($crm) && $crm->customer_type==$index)?'selected':''}}>{{$value}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="talking" class="form-control" name="main[talking]">
                                @foreach($talking as $index=>$value)
                                    <option
                                        value="{{$index}}" {{(isset($crm) && $crm->talking==$index)?'selected':''}}>{{$value}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="purchase_stage" class="form-control" name="main[purchase_stage]">
                                @foreach($purchage_state as $index=>$value)
                                    <option
                                        value="{{$index}}" {{(isset($crm) && $crm->purchase_stage==$index)?'selected':''}}>{{$value}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
